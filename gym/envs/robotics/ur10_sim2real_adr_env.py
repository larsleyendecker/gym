...
class Ur10Env(robot_custom_env.RobotEnv):
    ...
    def set_force_for_q(self, q_ctrl, only_grav_comp=False):
        ...
        for i in range(6):
            target_q[i], zo = lfilter(
                self.b, self.a, [self.ctrl_buffer[3][i]], zi=self.zi[i])
            self.zi[i] = zo.copy()
    ...
    def _get_obs(self):
        ...
        rpy = normalize_rad(rotations.mat2euler(x_mat)
                            + numpy.random.normal(0.0, self.pos_noise_std[3:]) * (numpy.pi/180)
                            + numpy.random.uniform(-self.pos_rand_uncor_bound[3:], self.pos_rand_uncor_bound[3:]) * (numpy.pi/180)
    ...
    def level_change(self, success_rate, level):
        ...
        if success_rate > self.upper_level_bound and self.level_episodes >= self.level_episodes_bound:
            level += 1
            with open(os.path.join(*[ADR_CONFIG_PATH, "env_config_ADR_{}.yml".format(self.worker_id)])) as infile:
                last_config = yaml.load(infile, Loader=yaml.FullLoader)
                next_config = last_config

            next_config["Learning"]["ADR"]["level"] = level

            next_config["randomize_kwargs"]["var_mass"] = self.randomize_kwargs["var_mass"] + self.var_mass_delta
            next_config["randomize_kwargs"]["var_damp"] = self.randomize_kwargs["var_damp"] + self.var_damp_delta
            next_config["randomize_kwargs"]["var_fr"] = self.randomize_kwargs["var_fr"] + self.var_fr_delta
            next_config["randomize_kwargs"]["var_grav_x_y"] = self.randomize_kwargs["var_grav_x_y"] + self.var_grav_x_y_delta
            next_config["randomize_kwargs"]["var_grav_z"] = self.randomize_kwargs["var_grav_z"] + self.var_grav_z_delta
            next_config["randomize_kwargs"]["var_body_pos"] = self.randomize_kwargs["var_body_pos"] + self.var_body_pos_delta
            next_config["randomize_kwargs"]["var_body_rot"] = self.randomize_kwargs["var_body_rot"] + self.var_body_rot_delta

            next_config["Domain_Randomization"]["vary_params"] = (self.vary_params + self.vary_params_delta).tolist()
            next_config["Domain_Randomization"]["pos_rand_uncor_bound"] = (self.pos_rand_uncor_bound + self.pos_rand_uncor_bound_delta).tolist()
            next_config["Domain_Randomization"]["ft_rand_uncor_bound"] = (self.ft_rand_uncor_bound + self.ft_rand_uncor_bound_delta).tolist()
            next_config["Domain_Randomization"]["pos_rand_cor_bound"] = (self.pos_rand_cor_bound + self.pos_rand_cor_bound_delta).tolist()
            next_config["Domain_Randomization"]["ft_rand_cor_bound"] = (self.ft_rand_cor_bound + self.ft_rand_cor_bound_delta).tolist()
        
            self.randomize_kwargs = next_config["randomize_kwargs"]
            
            self.vary_params = numpy.array(next_config["Domain_Randomization"]["vary_params"])
            self.pos_rand_uncor_bound = numpy.array(next_config["Domain_Randomization"]["pos_rand_uncor_bound"])
            self.ft_rand_uncor_bound = numpy.array(next_config["Domain_Randomization"]["ft_rand_uncor_bound"])
            self.pos_rand_cor_bound = numpy.array(next_config["Domain_Randomization"]["pos_rand_cor_bound"])
            self.ft_rand_cor_bound = numpy.array(next_config["Domain_Randomization"]["ft_rand_cor_bound"])

            with open(os.path.join(*[ADR_CONFIG_PATH, "env_config_ADR_{}.yml".format(self.worker_id)]), "w") as outfile:
